 ...
 async function run(input, config, error) {
  ...
  if (!source) {
    let question = {
      ...
      pageSize: 10,
      loop: false,
      source: (answers, input) => searchPlanters(answers, input, list),
    }
      ...
      .then((answers) => {
        ...
        if (!dir) {
          let question = {
            type: 'input',
            name: 'selectDir',
            message: 'Where do you to start your project?',
          }
          inquirer
            .prompt([question])
            .then((answers) => {
              dir = answers.selectDir;
              createPlanter(planters, source, dir, options, error);
            });
        }
      });
  } else {
...
async function createPlanter(planters, source, dir, options, error) {
...
function searchPlanters(answers, input = '', list) {
